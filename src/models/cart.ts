...
export async function getCartByUser(userId: number) {
  try {
    ...
    throw new Error(`Could not get cart for user ${userId}. Error: ${err}`);
  }
}
...
export async function addItemToCart(userId: number, product_id: number, quantity: number) {
  try {
    ...
    throw new Error(`Could not add item to cart. Error: ${err}`);
  }
}
...
export async function updateCartItem(itemId: number, quantity: number) {
  try {
    ...
    throw new Error(`Could not update cart item. Error: ${err}`);
  }
}
...
export async function removeCartItem(itemId: number) {
  try {
    ...
    throw new Error(`Could not remove cart item. Error: ${err}`);
  }
}
